...
public class Main {
	...
	public static void main(String[] args) throws JMException {
	    ...
	    /*if (os == 0){	//windowsの場合
	    	...
	    	Gmethod.stringWrite(Log, hdfs, dataName);
			Gmethod.stringWrite(Log, hdfs, date.toString());
	    }*/
		...
		CC(Seed, dataName, hdfs, objectives, gen, dpop, sc, func, Npop, CV, Rep, Pon, os);

	}

	static public void CC(int Seed, String dataName, String hdfs, int objectives, int gen,int dpop, JavaSparkContext sc, int func,int Npop,int CV, int Rep, int Pon ,int os){

		//読み込みファイル名
		String traFiles[][] = new String[Rep][CV];
	    String tstFiles[][] = new String[Rep][CV];
	    Gmethod.makeFile(dataName, traFiles, tstFiles, hdfs);

	    //データディレクトリ作成
	    String resultDir;
	    resultDir = Gmethod.makeDir(dataName, hdfs, func, Seed, os);
	    ...
	    if (os == 0){	//windowsの場合
	    	...
	    	Gmethod.writeExp(dataName, resultDir, hdfs, st);
	    }
		...
		for(int pp=0;pp<Pon;pp++){
			...
			for(int j=0;j<Rep; j++){
				...
				for(int i=0;i<CV; i++){
					...
					pall(tstFiles[j][i], sc, rand, objectives, gen, rdd, func, Npop, res, time, i, j, pp, os);
	...
	static public void pall(String testFile, JavaSparkContext sc, MersenneTwisterFast rnd, int objectives, int gen, JavaRDD<String> rdd,int func,int Npop, Resulton res, TimeWatch time ,int CV, int Rep, int Pon, int os){
